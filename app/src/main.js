    ...
    function updateInspectorEmpty() {
      ...
      document.getElementById("selectionDesc").textContent = storyDataError
        ? `当前使用内置演示投影数据。JSON 读取失败：${storyDataError}`
    ...
    function updateInspectorCharacter(trackId, characterId) {
      ...
      const roleBriefList = participationEvents
        ...
        .join(" / ");
      const actionSummary = participationEvents
        .slice(0, 3)
        .map((eventItem) => eventItem.action || eventItem.input)
        .filter(Boolean)
        .join(" ");
